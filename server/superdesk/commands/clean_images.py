...
class CleanImages(superdesk.Command):
    ...
    def run(self):
        try:
            ...
            archive_items = superdesk.get_resource_service('archive').get(None, {'type': 'picture'})
            self.__add_existing_files(used_images, archive_items)

            ingest_items = superdesk.get_resource_service('ingest').get(None, {'type': 'picture'})
            self.__add_existing_files(used_images, ingest_items)

            upload_items = superdesk.get_resource_service('upload').get(req=None, lookup={})
            ...
            print('Number of files in GridFs: ', current_files.count())
